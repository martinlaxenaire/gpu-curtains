export class Geometry {
  constructor({ widthSegments = 1, heightSegments = 1, useIndexing = true }) {
    this.type = 'Geometry'
    ...
    const nbTriangles = this.definition.count * 2

    this.verticesCount = this.useIndexing
      ? this.definition.width * 2 + 2 + (this.definition.height - 1) * (this.definition.width + 1)
      : nbTriangles * 3
    ...
    this.attributes = {
      uv: {
        ...
        // nb of triangles * 3 vertices per triangle * 2 coordinates per triangle
        //size: this.definition.width * this.definition.height * 3 * 2
        size: 2,
  ...
  setVerticesAndUVs() {
    ...
    if (this.useIndexing) {
      ...
      this.computeVerticesAndUVs()
    }
  }

  computeVerticesAndUVs() {
    ...
    for (let y = 0; y < this.definition.height; y++) {
      ...
      for (let x = 0; x < this.definition.width; x++) {
        ...
        // second vertex
        uv[uvOffset++] = u
        uv[uvOffset++] = v + 1 / this.definition.height

        position[positionOffset++] = (u - 0.5) * 2
        position[positionOffset++] = (v + 1 / this.definition.height - 0.5) * 2
        position[positionOffset++] = 0

        this.value[offset++] = u
        this.value[offset++] = v + 1 / this.definition.height
        this.value[offset++] = (u - 0.5) * 2
        this.value[offset++] = (v + 1 / this.definition.height - 0.5) * 2
        this.value[offset++] = 0
        ...
        // first vertex (equals first triangle third vertex)
        uv[uvOffset++] = u + 1 / this.definition.width
        uv[uvOffset++] = v
        ...
        // second vertex (equals first triangle second vertex)
        uv[uvOffset++] = u
        uv[uvOffset++] = v + 1 / this.definition.height
