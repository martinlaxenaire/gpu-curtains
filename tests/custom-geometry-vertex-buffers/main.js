...
window.addEventListener('load', async () => {
  ...
  const { Geometry, GPUCameraRenderer, GPUDeviceManager, Mesh, Vec3 } = await import(/* @vite-ignore */ path)
  ...
  // const verticesColors = new Float32Array(colors.map((value) => value))
  ...
  geometry.setAttribute({
    ...
    normalized: true, // normalize colors from [0, 255] range to  [0, 1] range
  })
