...
//#region src/extras/environmentMap/EnvironmentMap.ts
/**
...
var EnvironmentMap = class {
	...
	*/
	constructor(renderer, params = {}) {
		...
		if (params.lutTextureParams) params.lutTextureParams = {
			...lutTextureDefaultParams,
			...params.lutTextureParams
		};
		else params.lutTextureParams = lutTextureDefaultParams;
		if (params.diffuseTextureParams) params.diffuseTextureParams = {
			...diffuseTextureDefaultParams,
			...params.diffuseTextureParams
		};
		else params.diffuseTextureParams = diffuseTextureDefaultParams;
		if (params.specularTextureParams) params.specularTextureParams = {
			...specularTextureDefaultParams,
			...params.specularTextureParams
		};
		else params.specularTextureParams = specularTextureDefaultParams;
		this.options = params;
	...
	*/
	setRenderer(renderer) {
		if (this.renderer) this.renderer.environmentMaps.delete(this.uuid);
	...
	*/
	onRotationAxisChanged(callback) {
		if (callback) this._onRotationAxisChangedCallback = callback;
		return this;
	}
	/**
	...
	*/
	async computeBRDFLUTTexture() {
		...
		for (const renderer of this.renderer.deviceManager.renderers) {
			for (const [uuid, envMap] of renderer.environmentMaps) if (uuid !== this.uuid && envMap.lutTexture && envMap.lutTexture.size.width === this.lutTexture.size.width) {
		...
		let computeLUTPass = new ComputePass(this.renderer, {
			...
			shaders: { compute: { code: computeBRDFLUT } },
			uniforms: { params: { struct: { sampleCount: {
				type: "u32",
	...
	*/
	async computeSpecularCubemapFromHDRData(parsedHdr) {
		...
		let computeCubeMapPass = new ComputePass(this.renderer, {
			...
			autoRender: false,
			dispatchSize: [
				Math.ceil(this.cubemapTexture.size.width / 8),
				Math.ceil(this.cubemapTexture.size.height / 8),
				6
			],
			shaders: { compute: { code: computeCubemapFromHDR } },
			storages: { params: {
				visibility: ["compute"],
				struct: {
					hdrImageData: {
	...
	*/
	generateSpecularPMREMTexture(commandEncoder, mipBuffers) {
		if (!this.cubemapTexture.texture) {
			if (!this.renderer.production) throwWarning("EnvironmentMap: Could not generate the PMREM mips because the cubemap texture is not set:" + this.cubemapTexture);
			return;
		}
	...
	*/
	async computeDiffuseFromCubemap() {
		...
		let computeDiffusePass = new ComputePass(this.renderer, {
			...
			shaders: { compute: { code: computeDiffuseFromCubemap(this.cubemapTexture) } },
			uniforms: { params: { struct: {
				faceSize: {
					type: "u32",
	...
	*/
	async loadAndComputeFromHDR(url) {
		...
		const { width, height } = this.#hdrData ? this.#hdrData : {
			width: 1024,
			height: 512
		};
	...
	*/
	computeFromHDR() {
		if (this.#hdrData) this.computeSpecularCubemapFromHDRData(this.#hdrData).then(() => {
