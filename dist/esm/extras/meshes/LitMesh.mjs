...
//#region src/extras/meshes/LitMesh.ts
/**
...
* ```
*/
var LitMesh = class LitMesh extends Mesh {
	/**
	* LitMesh constructor
  ...
  return output;`
		...
		if (defaultParams.uniforms) defaultParams.uniforms = {
			...defaultParams.uniforms,
			material: materialUniform
		};
		else defaultParams.uniforms = { material: materialUniform };
		if (!defaultParams.textures) defaultParams.textures = [];
		if (!defaultParams.samplers) defaultParams.samplers = [];
		LitMesh.getMaterialTexturesDescriptors({
			shading,
			baseColorTexture,
			normalTexture,
			emissiveTexture,
			...
			if (textureDescriptor.sampler) {
				if (!defaultParams.samplers.find((s) => s.uuid === textureDescriptor.sampler.uuid)) defaultParams.samplers.push(textureDescriptor.sampler);
			}
		...
		if (useEnvMap) {
			...
			if (environmentMap.lutTexture) defaultParams.textures = [...defaultParams.textures, environmentMap.lutTexture];
			if (!defaultParams.samplers) defaultParams.samplers = [];
		...
		if (thickness) extensionsUsed.push("KHR_materials_volume");
		if (dispersion) extensionsUsed.push("KHR_materials_dispersion");
		if (sheenColor || sheenRoughness) extensionsUsed.push("KHR_materials_sheen");
		if (anisotropy !== void 0) extensionsUsed.push("KHR_materials_anisotropy");
		if (clearcoat) extensionsUsed.push("KHR_materials_clearcoat");
		if (iridescence) extensionsUsed.push("KHR_materials_iridescence");
		if (diffuseTransmission !== void 0) extensionsUsed.push("KHR_materials_diffuse_transmission");
		if (multiscatterColor !== void 0 || scatterAnisotropy !== void 0) extensionsUsed.push("KHR_materials_volume_scatter");
		...
		if (useEnvMap) environmentMap.onRotationAxisChanged(() => {
	...
	*/
	static getMaterialUniform(parameters) {
		const { shading, colorSpace, color, opacity, alphaCutoff, metallic, roughness, normalScale, occlusionIntensity, emissiveIntensity, emissiveColor, specularIntensity, specularColor, shininess, transmission, ior, dispersion, thickness, attenuationDistance, attenuationColor, multiscatterColor, scatterAnisotropy, sheenColor, sheenRoughness, anisotropy, anisotropyVector, clearcoat, clearcoatRoughness, clearcoatNormalScale, iridescence, iridescenceIOR, iridescenceThicknessRange, diffuseTransmission, diffuseTransmissionColor, environmentMap } = parameters;
		const baseUniformStruct = {
			...
			alphaCutoff: {
				type: "f32",
				value: alphaCutoff !== void 0 ? alphaCutoff : .5
			},
			occlusionIntensity: {
				type: "f32",
		...
		return {
			...
			struct: (() => {
				switch (shading) {
					case "Unlit": return baseUniformStruct;
					case "Lambert": return diffuseUniformStruct;
					case "Phong": return phongUniformStruct;
					default: return pbrUniformStruct;
				}
			})()
		};
	}
	...
	*/
	static getMaterialTexturesDescriptors(parameters) {
		const { shading, baseColorTexture, normalTexture, emissiveTexture, occlusionTexture, metallicRoughnessTexture, specularTexture, specularFactorTexture, specularColorTexture, transmissionThicknessTexture, transmissionTexture, thicknessTexture, sheenTexture, sheenColorTexture, sheenRoughnessTexture, anisotropyTexture, clearcoatTexture, clearcoatFactorTexture, clearcoatRoughnessTexture, clearcoatNormalTexture, iridescenceTexture, iridescenceFactorTexture, iridescenceThicknessTexture, diffuseTransmissionTexture, diffuseTransmissionFactorTexture, diffuseTransmissionColorTexture } = parameters;
		const baseTextures = [
			baseColorTexture,
			emissiveTexture,
			occlusionTexture
		...
		return (() => {
			switch (shading) {
				case "Unlit": return baseTextures;
				case "Lambert": return diffuseTextures;
				case "Phong": return specularTextures;
				default: return pbrTextures;
			}
		})().filter(Boolean);
	}
	/**
