...
var GPUCameraRenderer = class extends GPURenderer {
	...
	*/
	setMainRenderPasses() {
		...
		this.transmissionTarget = { sampler: new Sampler(this, {
	...
	*/
	setCamera(cameraParameters) {
		...
		this.useCamera(new PerspectiveCamera({
	...
	*/
	useCamera(camera) {
		...
		if (this.camera) {
			if (this.camera.parent && this.camera.parent.object3DIndex === this.scene.object3DIndex) this.camera.parent = null;
		...
		if (this.bindings.camera) {
			...
			for (const mesh of this.meshes) if ("modelViewMatrix" in mesh) mesh.camera = this.camera;
		}
		return this;
	}
	/**
	...
	updateCameraViewport() {
		...
		if (this.camera instanceof PerspectiveCamera && this.camera.forceAspect) {
			...
			this.setCameraViewport({
				width,
				height,
				top: (this.canvas.height - height) * .5,
				left: (this.canvas.width - width) * .5,
				minDepth: 0,
				maxDepth: 1
			});
	...
	*/
	resizeCamera() {
		...
		if (this.camera instanceof PerspectiveCamera) this.camera?.setPerspective({
			width,
			height,
			pixelRatio: this.pixelRatio
		});
	...
	setCameraViewport(viewport = null) {
		...
		if (!this.cameraViewport) {
			...
			if (this.viewport) {
				...
				this.cameraViewport.left = Math.max(0, (this.viewport.width - this.cameraViewport.width) * .5);
				this.cameraViewport.top = Math.max(0, (this.viewport.height - this.cameraViewport.height) * .5);
			}
	...
	*/
	onCameraMatricesChanged() {
		this.updateCameraBindings();
		for (const mesh of this.meshes) if ("modelViewMatrix" in mesh) mesh.shouldUpdateProjectionMatrixStack();
	}
	/**
	...
	*/
	#initLights() {
		if (!this.options.lights) this.options.lights = {
			maxAmbientLights: 2,
			maxDirectionalLights: 5,
			maxPointLights: 5,
			maxSpotLights: 5,
	...
	*/
	addLight(light) {
		...
		this.#updateLightsCount(light.type);
	}
	/**
	...
	*/
	onMaxLightOverflow(lightsType, lightIndex = 0) {
		if (!this.options.lights) {
			if (!this.production) throwWarning(`${this.options.label} (${this.type}): You are adding a light (${lightsType}) to a renderer that should not initially handle lights. The renderer bind group will be re-created. This should be avoided.`);
			this.#initLights();
			this.cameraLightsBindGroup.destroy();
			this.setCameraLightsBindGroup();
		} else {
			if (!this.production) throwWarning(`${this.options.label} (${this.type}): You are overflowing the current max lights count of '${this.lightsBindingParams[lightsType].max}' for this type of lights: ${lightsType}. This should be avoided by setting a larger ${"max" + lightsType.charAt(0).toUpperCase() + lightsType.slice(1)} when instancing your ${this.type}.`);
			...
			if (oldLightBinding) this.cameraLightsBindGroup.destroyBufferBinding(oldLightBinding);
			this.setLightsTypeBinding(lightsType);
			const lightBindingIndex = this.cameraLightsBindGroup.bindings.findIndex((binding) => binding.name === lightsType);
			if (lightBindingIndex !== -1) this.cameraLightsBindGroup.bindings[lightBindingIndex] = this.bindings[lightsType];
			else {
			...
			if (lightsType === "directionalLights" || lightsType === "pointLights" || lightsType === "spotLights") {
				...
				if (oldShadowsBinding) this.cameraLightsBindGroup.destroyBufferBinding(oldShadowsBinding);
				this.setShadowsTypeBinding(lightsType);
				const shadowsBindingIndex = this.cameraLightsBindGroup.bindings.findIndex((binding) => binding.name === shadowsType);
				if (shadowsBindingIndex !== -1) this.cameraLightsBindGroup.bindings[shadowsBindingIndex] = this.bindings[shadowsType];
				else {
			...
			this.lights.forEach((light) => {
				if (light.type === lightsType) light.reset();
			});
		}
	}
	/**
	...
	*/
	get shadowCastingLights() {
		return this.lights?.filter((light) => light.type === "directionalLights" || light.type === "pointLights" || light.type === "spotLights");
	}
	/**
	...
	*/
	setCameraLightsBindGroup() {
		...
		if (this.device) this.createCameraLightsBindGroup();
	}
	/**
	...
	*/
	destroyTransmissionTarget() {
		if (this.transmissionTarget.texture) {
			...
			this.scene.renderPassEntries.screen = this.scene.renderPassEntries.screen.filter((passEntry) => passEntry.label !== "Transmission scene screen render pass");
	...
	*/
	render(commandEncoder) {
		...
		if (this.cameraLightsBindGroup) this.cameraLightsBindGroup.needsPipelineFlush = false;
	}
	/**
