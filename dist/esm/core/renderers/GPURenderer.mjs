...
//#region src/core/renderers/GPURenderer.ts
/**
...
var GPURenderer = class {
	...
	*/
	constructor({ deviceManager, label, container, pixelRatio = 1, autoResize = true, context = {}, renderPass }) {
		...
		if (!deviceManager || !(deviceManager instanceof GPUDeviceManager)) throwError(label ? `${label} (${this.type}): no device manager or wrong device manager provided: ${typeof deviceManager} (${deviceManager?.constructor.name})` : `${this.type}: no device manager or wrong device manager provided: ${typeof deviceManager} (${deviceManager?.constructor.name})`);
		if (!label) label = `${this.constructor.name}${deviceManager.renderers.length}`;
		...
		if (!isOffscreenCanvas) {
			...
			this.resize();
			if (!isContainerCanvas) this.domElement.element.appendChild(this.canvas);
		}
		if (this.deviceManager.device) this.setContext();
	}
	/**
		...
		if (this.device) {
			try {
				...
				if (this.options.context.format !== preferredFormat) {
					...
					if (this.renderPass && this.renderPass.options.colorAttachments?.length) this.renderPass.options.colorAttachments[0].targetFormat = preferredFormat;
					this.configureContext();
				} else {
					this.context = null;
					console.error(e);
			...
			this.textures.forEach((texture) => {
				if (!texture.texture) texture.createTexture();
			});
	...
	*/
	copyBufferToBuffer({ srcBuffer, dstBuffer, commandEncoder }) {
		if (!srcBuffer || !srcBuffer.GPUBuffer) {
			throwWarning(`${this.options.label} (${this.type}): cannot copy to buffer because the source buffer has not been provided`);
			return null;
		}
		if (!dstBuffer) dstBuffer = new Buffer();
		if (!dstBuffer.GPUBuffer) dstBuffer.createBuffer(this, {
		...
		if (srcBuffer.GPUBuffer.mapState !== "unmapped") {
			throwWarning(`${this.options.label} (${this.type}): Cannot copy from ${srcBuffer.GPUBuffer} because it is currently mapped`);
			return;
		}
		if (dstBuffer.GPUBuffer.mapState !== "unmapped") {
			throwWarning(`${this.options.label} (${this.type}): Cannot copy from ${dstBuffer.GPUBuffer} because it is currently mapped`);
			return;
		}
	...
	*/
	importExternalTexture(source, label = "") {
		return this.deviceManager.device?.importExternalTexture({
			label,
			source
		});
	}
	...
	*/
	copyGPUTextureToTexture(gpuTexture, texture, commandEncoder) {
		if (gpuTexture.width !== texture.texture.width || gpuTexture.height !== texture.texture.height || gpuTexture.depthOrArrayLayers !== texture.texture.depthOrArrayLayers) return;
		commandEncoder.copyTextureToTexture({ texture: gpuTexture }, { texture: texture.texture }, [
			gpuTexture.width,
			gpuTexture.height,
			gpuTexture.depthOrArrayLayers
		]);
		if (texture.options.generateMips) this.generateMips(texture, commandEncoder);
	}
	/**
	...
	*/
	copyTextureToGPUTexture(texture, gpuTexture, commandEncoder) {
		if (gpuTexture.width !== texture.texture.width || gpuTexture.height !== texture.texture.height || gpuTexture.depthOrArrayLayers !== texture.texture.depthOrArrayLayers) return;
		commandEncoder.copyTextureToTexture({ texture: texture.texture }, { texture: gpuTexture }, [
			gpuTexture.width,
			gpuTexture.height,
			gpuTexture.depthOrArrayLayers
		]);
	...
	*/
	createSampler(sampler) {
		...
		if (existingSampler) return existingSampler.sampler;
		else {
	...
	*/
	onResize(callback) {
		if (callback) this._onResizeCallback = callback;
		return this;
	}
	/**
	...
	*/
	renderSingleComputePass(commandEncoder, computePass, copyBuffer = true) {
		...
		pass.end();
		if (copyBuffer) computePass.copyBufferToResult(commandEncoder);
	}
	/**
	...
	*/
	onAfterCommandEncoder() {
		if (!this.ready) return;
		this.textures.forEach((texture) => {
			if (texture instanceof MediaTexture) texture.closeVideoFrame();
		});
	...
	*/
	render(commandEncoder) {
		...
		if (this.shouldRenderScene) {
			this.textures.forEach((texture) => {
				if (texture instanceof MediaTexture) texture.update();
			});
