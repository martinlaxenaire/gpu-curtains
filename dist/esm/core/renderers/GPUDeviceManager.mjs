...
//#region src/core/renderers/GPUDeviceManager.ts
/**
...
var GPUDeviceManager = class {
	...
	*/
	constructor({ label, production = false, adapterOptions = {}, requiredFeatures = [], requestAdapterLimits = [], autoRender = true, onError = (message) => {}, onDeviceLost = (info) => {}, onDeviceDestroyed = (info) => {} } = {}) {
		...
		if (this.options.autoRender) this.animate();
	}
	/**
	...
	*/
	async init({ adapter = null, device = null } = {}) {
		...
		if (this.device) {
			this.#createSamplers();
			for (const renderer of this.renderers) if (!renderer.context) renderer.setContext();
		}
	}
	/**
	...
	*/
	async setDevice(device = null) {
		...
		if (device) {
			...
			if (this.options.adapterOptions.featureLevel === "compatibility") this.options.requestAdapterLimits.push("maxStorageBuffersInVertexStage", "maxStorageTexturesInVertexStage", "maxStorageBuffersInFragmentStage", "maxStorageTexturesInFragmentStage");
			const requiredLimits = {};
			for (const key in limits) if (this.options.requestAdapterLimits.includes(key)) requiredLimits[key] = limits[key];
			const requiredFeatures = [];
			this.options.requiredFeatures.forEach((feature) => {
				if (features.has(feature)) requiredFeatures.push(feature);
			});
	...
	*/
	loseDevice() {
		...
		this.deviceRenderedObjects.forEach((object) => {
			if (object.material && object.material.pipelineEntry) usedPipelineEntries.add(object.material.pipelineEntry.uuid);
		});
		this.pipelineManager.pipelineEntries = this.pipelineManager.pipelineEntries.filter((pipelineEntry) => usedPipelineEntries.has(pipelineEntry.uuid));
	...
	*/
	uploadTexture(texture, sourceIndex = 0) {
		if ("sources" in texture && texture.sources.length) try {
			...
			if (texture.texture.mipLevelCount > 1) this.generateMips(texture);
		...
		else for (let i = 0; i < texture.size.depth; i++) this.device?.queue.writeTexture({
			texture: texture.texture,
			origin: [
				0,
				0,
		...
		}, new Uint8Array(texture.options.placeholderColor), { bytesPerRow: texture.size.width * 4 }, {
			width: 1,
			height: 1,
			depthOrArrayLayers: 1
		});
	...
	*/
	generateMips(texture, commandEncoder = null) {
		...
		if (!this.#mipsGeneration.pipelineByFormatAndView[texture.texture.format + textureBindingViewDimension]) {
			...
			this.#mipsGeneration.pipelineByFormatAndView[texture.texture.format + textureBindingViewDimension] = this.device.createRenderPipeline({
				label: `Mip level generator pipeline for ${textureBindingViewDimension}, format: ${texture.texture.format}`,
				layout: "auto",
				vertex: { module: this.#mipsGeneration.module },
				fragment: {
		...
		const encoder = commandEncoder || this.device.createCommandEncoder({ label: "Mip gen encoder" });
		for (let baseMipLevel = 1; baseMipLevel < texture.texture.mipLevelCount; ++baseMipLevel) for (let layer = 0; layer < texture.texture.depthOrArrayLayers; ++layer) {
			const bindGroup = this.device.createBindGroup({
				layout: pipeline.getBindGroupLayout(0),
				entries: [{
					binding: 0,
					resource: this.#mipsGeneration.sampler
				}, {
					binding: 1,
