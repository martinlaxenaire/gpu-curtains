...
//#region src/core/meshes/mixins/MeshBaseMixin.ts
let meshIndex = 0;
	...
	return class MeshBase extends Base {
		...
		*/
		constructor(...params) {
			...
			const { label, shaders, geometry, visible, renderOrder, outputTarget, additionalOutputTargets, useCustomScenePassEntry, renderBundle, texturesOptions, autoRender, ...meshParameters } = parameters;
			...
			this.ready = false;
			this.userData = {};
			if (geometry) this.useGeometry(geometry);
			this.setMaterial({
		...
		*/
		addToScene(addToRenderer = false) {
			if (addToRenderer) this.renderer.meshes.push(this);
			...
			if (this.#autoRender) {
				...
				if (this.additionalOutputTargets.length) this.additionalOutputTargets.forEach((renderTarget) => {
		...
		*/
		removeFromScene(removeFromRenderer = false) {
			if (this.#autoRender) this.renderer.scene.removeMesh(this);
			if (removeFromRenderer) this.renderer.meshes = this.renderer.meshes.filter((m) => m.uuid !== this.uuid);
		}
		/**
		...
		*/
		setRenderer(renderer) {
			...
			if (!renderer || !(renderer.type === "GPURenderer" || renderer.type === "GPUCameraRenderer" || renderer.type === "GPUCurtainsRenderer")) {
				throwWarning(`${this.options.label}: Cannot set ${renderer} as a renderer because it is not of a valid Renderer type.`);
				return;
			}
			...
			this.renderer = renderer;
			this.addToScene(true);
			if (!oldRenderer.meshes.length) oldRenderer.onBeforeRenderScene.add((commandEncoder) => {
		...
		*/
		setRenderBundle(renderBundle, updateScene = true) {
			if (updateScene) {
				...
				this.addToScene();
			} else this.renderBundle = renderBundle;
		}
		/**
		...
		*/
		useGeometry(geometry) {
			if (this.geometry) {
				if (geometry.shouldCompute) geometry.computeGeometry();
				if (this.geometry.layoutCacheKey !== geometry.layoutCacheKey) {
					throwWarning(`${this.options.label} (${this.type}): the current and new geometries do not have the same vertexBuffers layout, causing a probable pipeline recompilation. This should be avoided.\n\nCurrent geometry layout:\n\n${this.geometry.wgslStructFragment}\n\n--------\n\nNew geometry layout:\n\n${geometry.wgslStructFragment}`);
					this.material.setAttributesFromGeometry(geometry);
					this.material.setPipelineEntry();
				}
				this.geometry.consumers.delete(this.uuid);
				if (this.options.renderBundle) this.options.renderBundle.ready = false;
			}
		...
		*/
		computeGeometry() {
			if (this.geometry.shouldCompute) this.geometry.computeGeometry();
		}
		/**
		...
		*/
		setGeometry() {
			if (this.geometry) {
				if (!this.geometry.ready) this.geometry.createBuffers({
		...
		*/
		setRenderingOptionsForRenderPass(renderPass) {
			const renderingOptions = {
				...
				...renderPass.options.colorAttachments.length && { targets: renderPass.options.colorAttachments.map((colorAttachment, index) => {
					return {
						format: colorAttachment.targetFormat,
						...this.options.targets?.length && this.options.targets[index] && this.options.targets[index].blend && { blend: this.options.targets[index].blend }
					};
				}) },
				depth: renderPass.options.useDepth,
				...renderPass.options.useDepth && { depthFormat: renderPass.options.depthFormat }
			};
		...
		*/
		useMaterial(material) {
			...
			if (this.material) {
				if (this.geometry) currentCacheKey = this.material.cacheKey;
				if (this.options.renderBundle) this.options.renderBundle.ready = false;
			}
			this.material = material;
			if (this.geometry) this.material.setAttributesFromGeometry(this.geometry);
			this.transparent = this.material.options.rendering.transparent;
			if (currentCacheKey && currentCacheKey !== this.material.cacheKey) if (this.material.ready) this.material.setPipelineEntry();
			else this.material.compileMaterial();
		}
		/**
		...
		*/
		setMaterialGeometryAttributes() {
			if (this.material && !this.material.attributes) this.material.setAttributesFromGeometry(this.geometry);
		}
		/**
		...
		*/
		set transparent(value) {
			...
			if (switchTransparency) this.removeFromScene();
			this._transparent = value;
			if (switchTransparency) this.addToScene();
		}
		/**
		* Get the visible property value.
		*/
		get visible() {
			if (super.visible !== void 0) return super.visible;
		...
		*/
		set visible(value) {
			if (super.visible !== void 0) super.visible = value;
		...
		*/
		resizeTextures() {
			this.textures?.forEach((texture) => {
				if (texture.options.fromTexture) texture.copy(texture.options.fromTexture);
			});
		}
		/**
		...
		*/
		remove() {
			this.removeFromScene(true);
			this.destroy();
			if (!this.renderer.meshes.length) this.renderer.onBeforeRenderScene.add((commandEncoder) => {
		...
		*/
		destroy() {
			...
			this.geometry.consumers.delete(this.uuid);
			if (!this.geometry.consumers.size) this.geometry?.destroy(this.renderer);
		}
	};
}
//#endregion
