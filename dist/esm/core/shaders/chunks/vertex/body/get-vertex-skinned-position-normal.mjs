//#region src/core/shaders/chunks/vertex/body/get-vertex-skinned-position-normal.ts
/**
...
const getVertexSkinnedPositionNormal = ({ bindings = [], geometry }) => {
	...
	if (geometry.vertexBuffers && geometry.vertexBuffers.length) geometry.vertexBuffers.forEach((vertexBuffer) => {
		vertexBuffer.attributes.forEach((attribute) => {
			if (attribute.name.includes("joints")) skinJoints.push(attribute);
			if (attribute.name.includes("weights")) skinWeights.push(attribute);
		});
	});
