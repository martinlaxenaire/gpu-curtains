...
//#region src/core/geometries/Geometry.ts
/**
...
var Geometry = class Geometry {
	...
	*/
	constructor({ verticesOrder = "ccw", topology = "triangle-list", instancesCount = 1, vertexBuffers = [], mapBuffersAtCreation = true } = {}) {
		...
		if (!vertexBuffers.length || !attributesBuffer) this.addVertexBuffer({ name: "attributes" });
		else if (attributesBuffer) vertexBuffers.sort((a, b) => {
			return (a.name !== "attributes" ? Infinity : -1) - (b.name !== "attributes" ? Infinity : -1);
		});
		for (const vertexBuffer of vertexBuffers) this.addVertexBuffer({
		...
		if (attributesBuffer) this.setWGSLFragment();
	}
	/**
	...
	*/
	static dequantize(typedArrayConstructor) {
		switch (typedArrayConstructor) {
			case Int8Array: return (v) => Math.max(v / 127, -1);
			case Uint8Array: return (v) => v / 255;
			case Int16Array: return (v) => Math.max(v / 32767, -1);
			case Uint16Array: return (v) => v / 65535;
			default: return (v) => v;
		}
	}
	/**
	...
	*/
	loseContext() {
		this.ready = false;
		for (const vertexBuffer of this.vertexBuffers) vertexBuffer.buffer.destroy();
	}
	/**
	...
	*/
	addVertexBuffer({ stepMode = "vertex", name, attributes = [], buffer = null, array = null, arrayBuffer = null, bufferOffset = 0, bufferSize = null } = {}) {
	...
	*/
	setAttribute({ vertexBuffer = this.vertexBuffers[0], name, type, bufferFormat, size = 3, array = new Float32Array(this.verticesCount * size), normalized = false, verticesStride = 1 }) {
		...
		if (name === "position") this.verticesCount = attributeCount;
		if (vertexBuffer.stepMode === "vertex" && this.verticesCount && this.verticesCount !== attributeCount * verticesStride) throwError(`Geometry vertex attribute error. Attribute ${name} array of size ${size} must be of length: ${this.verticesCount * size}, current given: ${array.length}. (${this.verticesCount} vertices).`);
		else if (vertexBuffer.stepMode === "instance" && attributeCount !== this.instancesCount) if (vertexBuffer.buffer) arrayLength = this.instancesCount * size;
		else throwError(`Geometry instance attribute error. Attribute array of size ${size} must be of length: ${this.instancesCount * size}, current given: ${array.length}. (${this.instancesCount} instances).`);
	...
	*/
	computeGeometry() {
		...
		this.vertexBuffers.forEach((vertexBuffer, index) => {
			...
			if (index === 0) {
				if (!hasPositionAttribute) throwError(`Geometry must have a 'position' attribute`);
				if (!vertexBuffer.attributes.find((attribute) => attribute.name === "normal")) {
			...
			if (vertexBuffer.array && vertexBuffer.array.byteLength === vertexBuffer.bufferLength) {
				...
				new Uint8Array(vertexBuffer.arrayBuffer).set(new Uint8Array(vertexBuffer.array.buffer, vertexBuffer.array.byteOffset, vertexBuffer.array.byteLength));
				...
				for (let a = 0; a < vertexBuffer.attributes.length; a++) {
					const { name, array, size, bufferOffset, verticesStride } = vertexBuffer.attributes[a];
					const setFunction = vertexBufferViewSetFunction(arrayView, array);
					const arrayLength = array.length;
					for (let i = 0; i < arrayLength; i += size) for (let s = 0; s < size; s++) {
						...
						for (let vs = 0; vs < verticesStride; vs++) {
							...
							setFunction((vs + (verticesStride - 1) * attrIndex) * vertexBuffer.arrayStride + attrStrideOffset + bufferOffset + attrOffset, attrValue, true);
						}
					}
				}
				if (hasPositionAttribute && hasPositionAttribute.array && hasPositionAttribute.normalized) {
					const dequantizePositions = Geometry.dequantize(hasPositionAttribute.array.constructor);
		...
		if (!this.wgslStructFragment) this.setWGSLFragment();
	}
	/**
	...
	*/
	createBuffers({ renderer, label = this.type }) {
		if (this.ready) return;
		for (const vertexBuffer of this.vertexBuffers) {
			if (!vertexBuffer.bufferSize) vertexBuffer.bufferSize = vertexBuffer.arrayBuffer.byteLength;
	...
	*/
	uploadBuffer(renderer, buffer) {
		if (this.options.mapBuffersAtCreation) {
			...
			} else if (buffer.array) new buffer.array.constructor(buffer.buffer.GPUBuffer.getMappedRange()).set(buffer.array);
			buffer.buffer.GPUBuffer.unmap();
		} else renderer.queueWriteBuffer(buffer.buffer.GPUBuffer, 0, buffer.arrayBuffer ?? buffer.array);
	}
	/**
	...
	*/
	drawGeometry(pass) {
		if (this.indirectDraw && this.indirectDraw.buffer && this.indirectDraw.buffer.GPUBuffer) pass.drawIndirect(this.indirectDraw.buffer.GPUBuffer, this.indirectDraw.offset);
		else pass.draw(this.verticesCount, this.instancesCount);
	}
	/**
	...
	*/
	destroy(renderer = null) {
		...
		for (const vertexBuffer of this.vertexBuffers) {
			...
			if (!vertexBuffer.buffer.consumers.size) vertexBuffer.buffer.destroy();
