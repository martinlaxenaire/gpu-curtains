...
//#region src/core/cameras/PerspectiveCamera.ts
/**
...
var PerspectiveCamera = class extends Camera {
	...
	*/
	constructor({ label = "Perspective camera", fov = 50, near = .1, far = 150, width = 1, height = 1, pixelRatio = 1, forceAspect = false, onMatricesChanged = () => {} } = {}) {
		super({
			label,
			near,
			far,
	...
	*/
	setSize({ width, height }) {
		if (width !== this.size.width || height !== this.size.height) this.shouldUpdateProjectionMatrices();
	...
	*/
	setCSSPerspective() {
		this.CSSPerspective = Math.pow(Math.pow(this.size.width / (2 * this.pixelRatio), 2) + Math.pow(this.size.height / (2 * this.pixelRatio), 2), .5) / Math.tan(this.fov * .5 * Math.PI / 180);
	}
	/**
