...
//#region src/core/pipelines/ComputePipelineEntry.ts
/**
...
var ComputePipelineEntry = class extends PipelineEntry {
	...
	*/
	patchShaders() {
		...
		if (this.options.shaders.compute.constants) for (const [key, value] of Object.entries(this.options.shaders.compute.constants)) this.shaders.compute.constants.set(key, value);
		...
		for (const groupBinding of groupsBindings) {
			if (groupBinding.wgslStructFragment && this.shaders.compute.head.indexOf(groupBinding.wgslStructFragment) === -1) this.shaders.compute.head = `\n${groupBinding.wgslStructFragment}\n${this.shaders.compute.head}`;
			if (this.shaders.compute.head.indexOf(groupBinding.wgslGroupFragment) === -1) this.shaders.compute.head = `${this.shaders.compute.head}\n@group(${groupBinding.groupIndex}) @binding(${groupBinding.bindIndex}) ${groupBinding.wgslGroupFragment}`;
	...
	createPipelineDescriptor() {
		...
		this.descriptor = {
			...
			compute: {
				...
				...this.options.shaders.compute.constants && { constants: this.options.shaders.compute.constants }
			}
		};
	}
	/**
	...
	*/
	async compilePipelineEntry() {
		...
		if (this.options.useAsync) await this.createComputePipelineAsync();
		else {
