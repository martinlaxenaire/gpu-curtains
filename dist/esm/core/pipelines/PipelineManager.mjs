...
//#region src/core/pipelines/PipelineManager.ts
/**
...
var PipelineManager = class {
	...
	*/
	get renderPipelines() {
		return this.pipelineEntries.filter((pipelineEntry) => pipelineEntry instanceof RenderPipelineEntry);
	}
	/**
	...
	*/
	get computePipelines() {
		return this.pipelineEntries.filter((pipelineEntry) => pipelineEntry instanceof ComputePipelineEntry);
	}
	/**
	...
	*/
	isSameRenderPipeline(parameters) {
		...
		for (const renderPipeline of renderPipelines) {
			...
			if (compareRenderingOptions(rendering, options.rendering).length) continue;
			if (!this.compareShaders(shaders.vertex, options.shaders.vertex)) continue;
			if (!(!shaders.fragment && !options.shaders.fragment || this.compareShaders(shaders.fragment, options.shaders.fragment))) continue;
	...
	*/
	isSameComputePipeline(parameters) {
		...
		for (const computePipeline of computePipelines) {
			...
			if (cacheKey !== options.cacheKey) continue;
			if (!this.compareShaders(shaders.compute, options.shaders.compute)) continue;
	...
	*/
	createRenderPipeline(material) {
		...
		if (existingPipelineEntry) return existingPipelineEntry;
		else {
