...
import { ProjectedShaderChunks, shaderChunks } from "../shaders/shader-chunks.mjs";
//#region src/core/pipelines/RenderPipelineEntry.ts
/**
...
* ```
*/
var RenderPipelineEntry = class RenderPipelineEntry extends PipelineEntry {
	/**
	...
	patchShaders() {
		...
		if (this.options.shaders.fragment) {
			for (const chunk in shaderChunks.fragment) {
				...
				if (this.shaders.full.head.indexOf(shaderChunks.fragment[chunk]) === -1) this.shaders.full.head = `${shaderChunks.fragment[chunk]}\n${this.shaders.full.head}`;
			}
			if (this.options.shaders.fragment.constants) for (const [key, value] of Object.entries(this.options.shaders.fragment.constants)) this.shaders.fragment.constants.set(key, value);
		}
		if (this.options.rendering.useProjection) {
			...
			if (this.options.shaders.fragment) for (const chunk in ProjectedShaderChunks.fragment) {
				this.shaders.fragment.head = `${ProjectedShaderChunks.fragment[chunk]}\n${this.shaders.fragment.head}`;
				if (this.shaders.full.head.indexOf(ProjectedShaderChunks.fragment[chunk]) === -1) this.shaders.full.head = `${ProjectedShaderChunks.fragment[chunk]}\n${this.shaders.full.head}`;
			}
		}
		if (this.options.shaders.vertex.constants) for (const [key, value] of Object.entries(this.options.shaders.vertex.constants)) this.shaders.vertex.constants.set(key, value);
		...
		for (const groupBinding of groupsBindings) {
			if (groupBinding.visibility.includes("vertex")) {
				if (groupBinding.wgslStructFragment && this.shaders.vertex.head.indexOf(groupBinding.wgslStructFragment) === -1) this.shaders.vertex.head = `\n${groupBinding.wgslStructFragment}\n${this.shaders.vertex.head}`;
			...
			if (this.options.shaders.fragment && groupBinding.visibility.includes("fragment")) {
				if (groupBinding.wgslStructFragment && this.shaders.fragment.head.indexOf(groupBinding.wgslStructFragment) === -1) this.shaders.fragment.head = `\n${groupBinding.wgslStructFragment}\n${this.shaders.fragment.head}`;
			...
			if (groupBinding.wgslStructFragment && this.shaders.full.head.indexOf(groupBinding.wgslStructFragment) === -1) this.shaders.full.head = `\n${groupBinding.wgslStructFragment}\n${this.shaders.full.head}`;
		...
		if (this.options.shaders.fragment) this.shaders.fragment.code = this.shaders.fragment.head + this.options.shaders.fragment.code;
		if (this.options.shaders.fragment) if (this.options.shaders.vertex.entryPoint !== this.options.shaders.fragment.entryPoint && this.options.shaders.vertex.code.localeCompare(this.options.shaders.fragment.code) === 0) this.shaders.full.code = this.shaders.full.head + this.options.shaders.vertex.code;
		else this.shaders.full.code = this.shaders.full.head + this.options.shaders.vertex.code + this.options.shaders.fragment.code;
	}
	/**
	...
	*/
	createShaders() {
		...
		if (this.options.shaders.fragment) this.shaders.fragment.module = this.createShaderModule({
	...
	*/
	createPipelineDescriptor() {
		...
		if (this.options.rendering.targets.length) {
			if (this.options.rendering.transparent) this.options.rendering.targets[0].blend = this.options.rendering.targets[0].blend ? this.options.rendering.targets[0].blend : RenderPipelineEntry.getDefaultTransparentBlending();
		} else this.options.rendering.targets = [];
		this.descriptor = {
			...
			...this.options.shaders.fragment && { fragment: {
			...
			primitive: {
				...
				...this.options.rendering.stripIndexFormat && { stripIndexFormat: this.options.rendering.stripIndexFormat }
			},
			...this.options.rendering.depth && { depthStencil: {
	...
	*/
	async compilePipelineEntry() {
		...
		if (this.options.useAsync) await this.createRenderPipelineAsync();
		else {
