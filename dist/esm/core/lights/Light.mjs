...
//#region src/core/lights/Light.ts
...
var Light = class extends Object3D {
	...
	*/
	setRenderer(renderer) {
		...
		if (this.renderer) this.renderer.removeLight(this);
		renderer = isCameraRenderer(renderer, this.type);
		this.renderer = renderer;
		if (this.index === void 0) this.index = this.renderer.lights.filter((light) => light.type === this.type).length;
		if (!this.renderer.lightsBindingParams || this.index + 1 > this.renderer.lightsBindingParams[this.type].max) this.onMaxLightOverflow(this.type);
		this.renderer.addLight(this);
		this.setRendererBinding();
		if (hasRenderer) this.reset(false);
	}
	/**
	...
	*/
	setRendererBinding() {
		if (this.renderer.bindings[this.type]) this.rendererBinding = this.renderer.bindings[this.type];
	}
	/**
	...
	*/
	onPropertyChanged(propertyKey, value) {
		if (this.rendererBinding && this.rendererBinding.inputs[propertyKey]) {
			...
			} else this.rendererBinding.inputs[propertyKey].value[this.index] = value;
	...
	*/
	onMaxLightOverflow(lightsType) {
		...
		if (this.rendererBinding) this.rendererBinding = this.renderer.bindings[lightsType];
	}
	/** @ignore */
	applyRotation() {}
	/** @ignore */
