import { BufferElement } from "./BufferElement.mjs";
//#region src/core/bindings/bufferElements/BufferArrayElement.ts
/**
...
var BufferArrayElement = class extends BufferElement {
	...
	*/
	setAlignment(startOffset = 0, minStride = 0) {
		if (minStride !== 0) startOffset = Math.ceil(startOffset / minStride) * minStride;
		...
		if (minStride !== 0) this.alignment.end.byte = Math.max(this.alignment.end.byte, minStride - 1);
	}
	/**
	...
	*/
	setValueFromArray(value) {
		...
		for (let i = 0; i < this.numElements; i++) for (let j = 0; j < this.bufferLayout.numElements; j++) {
