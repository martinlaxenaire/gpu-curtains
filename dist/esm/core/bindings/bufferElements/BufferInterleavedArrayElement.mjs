...
//#region src/core/bindings/bufferElements/BufferInterleavedArrayElement.ts
/**
	...
	*/
	update(value) {
		...
		for (let i = 0; i < this.numElements; i++) {
			const subarray = this.view.subarray(i * this.bufferLayout.numElements, i * this.bufferLayout.numElements + this.bufferLayout.numElements);
	...
	*/
	extractDataFromBufferResult(result) {
		...
		for (let i = 0; i < this.numElements; i++) {
			...
			for (let j = 0; j < this.bufferLayout.numElements; j++) interleavedResult[i * this.bufferLayout.numElements + j] = result[resultOffset + j];
		}
