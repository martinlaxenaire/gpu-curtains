...
//#region src/core/textures/MediaTexture.ts
...
* ```
*/
var MediaTexture = class MediaTexture extends Texture {
	...
	*/
	constructor(renderer, parameters = defaultMediaTextureParams) {
		...
		const { useTransform, placeholderColor, useExternalTextures, cache, ...baseTextureParams } = {
		...
		super(renderer, {
			...
			aspect: "all",
			fixedSize: {
				width: parameters.fixedSize?.width ?? 1,
	...
	*/
	updateModelMatrix() {
		if (this.options.useTransform) {
			this.modelMatrix.setUVTransform(this.offset.x, this.offset.y, this.scale.x, this.scale.y, this.rotation, this.transformOrigin.x, this.transformOrigin.y);
			this.transformBinding.inputs.matrix.shouldUpdate = true;
		} else throwWarning(`Texture: Cannot update ${this.options.name} transformation since its useTransform property has been set to false. You should set it to true when creating the Texture.`);
	}
	/**
	* Set our {@link Texture#bindings | bindings}.
	*/
	setBindings() {
		this.bindings = [new TextureBinding({
	...
	*/
	copy(texture) {
		if (this.size.depth !== texture.size.depth) {
			throwWarning(`${this.options.label}: cannot copy a ${texture.options.label} because the depth sizes differ: ${this.size.depth} vs ${texture.size.depth}.`);
			return;
		}
	...
	*/
	createTexture() {
		...
		if (this.options.fromTexture && (!(this.options.fromTexture instanceof MediaTexture) || this.options.fromTexture.sourcesUploaded)) {
	...
	*/
	setSourceSize() {
		...
		this.options.fixedSize.width = Math.max(1, source.source.naturalWidth || source.source.width || source.source.videoWidth);
		this.options.fixedSize.height = Math.max(1, source.source.naturalHeight || source.source.height || source.source.videoHeight);
	...
	*/
	async loadImageBitmap(url) {
		if (url.includes(".webp")) return new Promise((resolve, reject) => {
			const img = new Image();
	...
	*/
	async loadImage(source) {
		...
		if (this.options.cache) {
			const cachedTexture = this.renderer.textures.filter((t) => t instanceof MediaTexture && t.uuid !== this.uuid).find((t) => {
	...
	*/
	useImageBitmap(imageBitmap, sourceIndex = 0) {
		if (this.size.depth > 1) this.sources[sourceIndex] = {
			source: imageBitmap,
			externalSource: null,
			sourceLoaded: true,
	...
	*/
	async loadImages(sources) {
		for (let i = 0; i < Math.min(this.size.depth, sources.length); i++) this.loadImage(sources[i]);
	}
	/**
	...
	*/
	closeVideoFrame() {
		const source = this.sources[0];
		if (source && source.externalSource) source.externalSource.close();
	}
	/**
	...
	*/
	onVideoFrameCallback(sourceIndex = 0) {
		if (this.videoFrameCallbackIds.get(sourceIndex)) {
			...
			this.sources[sourceIndex].source.requestVideoFrameCallback(this.onVideoFrameCallback.bind(this, sourceIndex));
		}
	}
	/**
	...
	*/
	onVideoLoaded(video, sourceIndex = 0) {
		if (!this.sources[sourceIndex].sourceLoaded) {
			if (this.options.sources[sourceIndex] instanceof MediaStream && video.paused) video.addEventListener("play", () => {
	...
	*/
	loadVideo(source) {
		...
		if (isNaN(video.duration)) video.load();
	}
	/**
	...
	*/
	useVideo(video, sourceIndex = 0) {
		...
		if (video.readyState >= video.HAVE_ENOUGH_DATA) this.onVideoLoaded(video, sourceIndex);
		else video.addEventListener("canplaythrough", this.onVideoLoaded.bind(this, video, sourceIndex), { once: true });
	}
	/**
	...
	*/
	loadVideos(sources) {
		for (let i = 0; i < Math.min(this.size.depth, sources.length); i++) this.loadVideo(sources[i]);
	}
	/**
	...
	*/
	loadCanvases(sources) {
		for (let i = 0; i < Math.min(this.size.depth, sources.length); i++) this.loadCanvas(sources[i]);
	}
	/**
	...
	*/
	onAllSourcesLoaded(callback) {
		if (callback) this._onAllSourcesLoadedCallback = callback;
		return this;
	}
	/**
	...
	*/
	onAllSourcesUploaded(callback) {
		if (callback) this._onAllSourcesUploadedCallback = callback;
		return this;
	}
	/**
	...
	* */
	update() {
		this.sources?.forEach((source, sourceIndex) => {
			...
			if (sourceType === "externalVideo") source.shouldUpdate = true;
			if (source.shouldUpdate && sourceType !== "externalVideo") {
				if (this.size.width !== this.texture.width || this.size.height !== this.texture.height || this.options.generateMips && this.texture && this.texture.mipLevelCount <= 1) this.createTexture();
	...
	*/
	destroy() {
		if (this.videoFrameCallbackIds.size) for (const [sourceIndex, id] of this.videoFrameCallbackIds) this.sources[sourceIndex].source.cancelVideoFrameCallback(id);
		this.sources.forEach((source) => {
			if (this.isVideoSource(source.source)) source.source.removeEventListener("canplaythrough", this.onVideoLoaded.bind(this, source.source), { once: true });
		});
		super.destroy();
	}
};
