...
//#region src/core/textures/Texture.ts
...
var Texture = class {
	...
	*/
	constructor(renderer, parameters = defaultTextureParams) {
		...
		if (this.options.generateMips) this.options.useMips = true;
		if (this.options.format === "rgba32float" && this.renderer.device && !this.renderer.device.features.has("float32-filterable")) this.options.format = "rgba16float";
		...
		if (!this.options.format) this.options.format = this.renderer.options.context.format;
		...
		if (this.options.fixedSize) this.#autoResize = false;
		this.setBindings();
	...
	*/
	setRenderer(renderer) {
		if (this.renderer) this.renderer.removeTexture(this);
		renderer = isRenderer(renderer, this.options.label + " Texture");
		this.renderer = renderer;
		this.renderer.addTexture(this);
		const { width, height } = this.renderer.canvas;
		if (this.#autoResize && (this.size.width !== width * this.options.qualityRatio || this.size.height !== height * this.options.qualityRatio)) this.resize();
	}
	/**
	...
	*/
	uploadSource({ source, width = this.size.width, height = this.size.height, depth = this.size.depth, origin = [
		0,
		0,
		0
	], colorSpace = "srgb" }) {
		...
		if (this.texture.mipLevelCount > 1) this.renderer.generateMips(this);
	}
	/**
	...
	*/
	uploadData({ width = this.size.width, height = this.size.height, depth = this.size.depth, origin = [
		0,
		0,
		0
		...
		this.renderer.device.queue.writeTexture({
			...
			bytesPerRow: width * data.BYTES_PER_ELEMENT * 4,
			rowsPerImage: height
		}, [
			width,
			height,
	...
	*/
	resize(size = null) {
		...
		if (size.width === this.size.width && size.height === this.size.height && size.depth === this.size.depth) return;
		this.setSize(size);
	}
	/**
	...
	*/
	destroy() {
		...
		if (!this.options.fromTexture) this.texture?.destroy();
		this.texture = null;
	}
};
//#endregion
