...
//#region src/core/bindGroups/BindGroup.ts
/**
...
var BindGroup = class {
	...
	*/
	addBinding(binding) {
		if ("buffer" in binding) {
			...
			if ("resultBuffer" in binding) binding.resultBuffer.consumers.add(this.uuid);
			this.bufferBindings.push(binding);
			if (binding.bindingType === "uniform") this.uniforms[binding.name] = binding.inputs;
			else this.storages[binding.name] = binding.inputs;
		}
	...
	*/
	destroyBufferBinding(binding) {
		if ("buffer" in binding) {
			...
			if (!binding.buffer.consumers.size) binding.buffer.destroy();
			if (binding.parent) {
		...
		if ("resultBuffer" in binding) {
			...
			if (!binding.resultBuffer.consumers.size) binding.resultBuffer.destroy();
		}
	}
	/**
	...
	*/
	createInputBindings(bindingType = "uniform", inputs = {}) {
		let bindings = [...Object.keys(inputs).map((inputKey) => {
			...
			if (binding.useStruct !== false) {
				...
				if (binding.shouldCopyResult !== void 0) key += `${binding.shouldCopyResult},`;
				const cachedBinding = this.renderer.deviceManager.bufferBindings.get(key);
				if (cachedBinding) return cachedBinding.clone(bindingParams);
			}
	...
	*/
	setInputBindings() {
		this.addBindings([...this.createInputBindings("uniform", this.options.uniforms), ...this.createInputBindings("storage", this.options.storages)]);
	}
	/**
	...
	*/
	resetBindGroup() {
		...
		this.pipelineCacheKey = "";
		for (const binding of this.bindings) this.addBindGroupEntry(binding);
		this.setBindGroup();
	}
	/**
	...
	*/
	resetBindGroupLayout() {
		...
		this.layoutCacheKey = "";
		for (const binding of this.bindings) this.addBindGroupLayoutEntry(binding);
		this.setBindGroupLayout();
	}
	/**
	...
	*/
	loseContext() {
		...
		for (const binding of this.bufferBindings) {
			...
			if (binding.parent) binding.parent.buffer.reset();
			if ("resultBuffer" in binding) binding.resultBuffer.reset();
		}
		this.bindGroup = null;
		this.bindGroupLayout = null;
	...
	*/
	restoreContext() {
		if (this.shouldCreateBindGroup) this.createBindGroup();
		for (const bufferBinding of this.bufferBindings) bufferBinding.shouldUpdate = true;
	}
	/**
	...
	createBindingBuffer(binding, optionalLabel = null) {
		binding.buffer.createBuffer(this.renderer, {
			...
			usage: [...[
				...
				"copyDst",
				binding.bindingType
			], ...binding.options.usage]
		});
		if ("resultBuffer" in binding) binding.resultBuffer.createBuffer(this.renderer, {
	...
	*/
	fillEntries() {
		for (const binding of this.bindings) {
			if (!binding.visibility) binding.visibility = GPUShaderStage.VERTEX | GPUShaderStage.FRAGMENT | GPUShaderStage.COMPUTE;
			if ("buffer" in binding) {
				if (binding.parent && !binding.parent.buffer.GPUBuffer) this.createBindingBuffer(binding.parent, binding.parent.options.label);
				else if (!binding.buffer.GPUBuffer && !binding.parent) this.createBindingBuffer(binding);
			}
	...
	*/
	setBindGroupLayout() {
		...
		if (bindGroupLayout) this.bindGroupLayout = bindGroupLayout;
		else {
		...
		this.bindings.forEach((binding, index) => {
			if ("buffer" in binding) {
				...
				if (binding.shouldUpdate && binding.buffer.GPUBuffer) {
					if (!binding.useStruct && binding.bufferElements.length > 1) this.renderer.queueWriteBuffer(binding.buffer.GPUBuffer, 0, binding.bufferElements[index].view);
					else this.renderer.queueWriteBuffer(binding.buffer.GPUBuffer, 0, binding.arrayBuffer);
	...
	*/
	update() {
		...
		if (needBindGroupReset || needBindGroupLayoutReset) this.renderer.onAfterCommandEncoderSubmission.add(() => {
			for (const binding of this.bindings) {
		...
		if (needBindGroupReset) this.resetBindGroup();
	}
	/**
	...
	*/
	clone({ bindings = [], keepLayout = false } = {}) {
		...
		params.label += " (copy)";
		const bindGroupCopy = new this.constructor(this.renderer, { label: params.label });
		...
		for (const binding of bindingsRef) {
			...
			if ("buffer" in binding) {
				...
				if ("resultBuffer" in binding) binding.resultBuffer.consumers.add(bindGroupCopy.uuid);
			}
			if (!keepLayout) bindGroupCopy.addBindGroupLayoutEntry(binding);
	...
	*/
	destroy() {
		...
		for (const binding of this.bufferBindings) this.destroyBufferBinding(binding);
		this.bindings = [];
		this.bindGroupLayout = null;
		this.bindGroup = null;
		this.resetEntries();
