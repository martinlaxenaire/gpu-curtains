...
//#region src/core/materials/ComputeMaterial.ts
/**
...
var ComputeMaterial = class extends Material {
	...
	*/
	constructor(renderer, parameters) {
		...
		if (!shaders || !shaders.compute) shaders = { compute: {
			code: "",
			entryPoint: "main"
		} };
		if (!shaders.compute.code) shaders.compute.code = "@compute @workgroup_size(1) fn main(){}";
		if (!shaders.compute.entryPoint) shaders.compute.entryPoint = "main";
		this.options = {
			...this.options,
			shaders,
			...parameters.dispatchSize !== void 0 && { dispatchSize: parameters.dispatchSize }
		};
		if (!dispatchSize) dispatchSize = 1;
		...
		} else if (!isNaN(dispatchSize)) dispatchSize = [
			Math.ceil(dispatchSize),
			1,
			1
		];
	...
	*/
	async compileMaterial() {
		...
		if (!this.pipelineEntry) this.setPipelineEntry();
	...
	*/
	useCustomRender(callback) {
		if (callback) this._useCustomRenderCallback = callback;
	}
	/**
	...
	*/
	render(pass) {
		...
		if (this._useCustomRenderCallback !== void 0) this._useCustomRenderCallback(pass);
		else {
			for (const bindGroup of this.bindGroups) pass.setBindGroup(bindGroup.index, bindGroup.bindGroup);
	...
	*/
	copyBufferToResult(commandEncoder) {
		for (const bindGroup of this.bindGroups) bindGroup.bufferBindings.forEach((binding) => {
			if (binding.shouldCopyResult) this.renderer.copyBufferToBuffer({
	...
	*/
	async getComputeResult({ bindingName = "", bufferElementName = "" }) {
		...
		if (binding && "resultBuffer" in binding) {
			...
			if (bufferElementName && result.length) return binding.extractBufferElementDataFromBufferResult({
				result,
				bufferElementName
			});
			else return result;
