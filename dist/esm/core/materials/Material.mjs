...
//#region src/core/materials/Material.ts
/**
...
var Material = class {
	...
	*/
	setRenderer(renderer) {
		...
		if (this.pipelineEntry) this.pipelineEntry.setRenderer(this.renderer);
	}
	/**
	...
	*/
	async compileMaterial() {
		const createBindGroups = () => {
			...
			if (!(this.bindGroups.length >= this.inputsBindGroups.length + texturesBindGroupLength)) this.createBindGroups();
		};
		if (this.renderer.ready) createBindGroups();
	...
	*/
	loseContext() {
		...
		].forEach((bindGroup) => bindGroup.loseContext());
	...
	*/
	restoreContext() {
		...
		for (const texture of this.textures) {
			if (texture instanceof MediaTexture) texture.sources.forEach((source) => {
				if (source.sourceLoaded) source.shouldUpdate = true;
			});
	...
	async getShaderCode(shaderType = "full") {
		shaderType = (() => {
			switch (shaderType) {
				...
				case "fragment":
				case "compute":
				case "full": return shaderType;
				default: return "full";
			}
		})();
		if (this.pipelineEntry) return this.pipelineEntry.shaders[shaderType].code;
	...
	async getAddedShaderCode(shaderType = "vertex") {
		shaderType = (() => {
			switch (shaderType) {
				case "vertex":
				case "fragment":
				case "compute": return shaderType;
				default: return "vertex";
			}
		})();
		if (this.pipelineEntry) return this.pipelineEntry.shaders[shaderType].head;
		else return new Promise((resolve) => {
			const taskId = this.renderer.onBeforeRenderScene.add(() => {
				if (this.pipelineEntry) {
					this.renderer.onBeforeRenderScene.remove(taskId);
	...
	*/
	processBindGroupBindings(bindGroup) {
		for (const inputBinding of bindGroup.bindings) this.inputsBindings.set(inputBinding.name, inputBinding);
		this.uniforms = {
			...this.uniforms,
			...bindGroup.uniforms
		};
	...
	*/
	createBindGroups() {
		...
		for (const bindGroup of this.inputsBindGroups) if (bindGroup.shouldCreateBindGroup) {
		...
		this.options.bindGroups?.forEach((bindGroup) => {
			...
			if (bindGroup instanceof TextureBindGroup && !this.texturesBindGroups.find((bG) => bG.uuid === bindGroup.uuid)) {
				...
				for (const texture of bindGroup.textures) if (!this.textures.find((t) => t.uuid !== texture.uuid)) this.textures.push(texture);
			}
		});
	}
	/**
	...
	*/
	destroyBindGroup(bindGroup) {
		...
		if (!bindGroup.consumers.size) bindGroup.destroy();
	}
	/**
	...
	*/
	updateBindGroups() {
		for (const bindGroup of this.bindGroups) this.updateBindGroup(bindGroup);
	}
	/**
	...
	*/
	shouldUpdateInputsBindings(bufferBindingName, bindingName) {
		...
		if (bufferBinding) if (!bindingName) Object.keys(bufferBinding.inputs).forEach((bindingKey) => bufferBinding.shouldUpdateBinding(bindingKey));
		else bufferBinding.shouldUpdateBinding(bindingName);
	}
	/**
	...
	*/
	setTextures() {
		this.textures = [];
		this.texturesBindGroups.push(new TextureBindGroup(this.renderer, { label: this.options.label + ": Textures bind group" }));
	...
	*/
	addTexture(texture) {
		...
		if (texture instanceof MediaTexture && texture.options.useTransform) texture.transformBinding.inputs.matrix.shouldUpdate = true;
	}
	/**
	...
	*/
	destroyTexture(texture) {
		...
		if (!objectsUsingTexture || !objectsUsingTexture.some((object) => object.material.uuid !== this.uuid)) texture.destroy();
	}
	/**
	...
	*/
	setSamplers() {
		...
		if (!this.samplers.find((sampler) => sampler.name === "defaultSampler")) {
	...
	*/
	addSampler(sampler) {
		...
		if (this.options.shaders && this.options.shaders.vertex && this.options.shaders.vertex.code.indexOf(sampler.name) !== -1 || this.options.shaders && this.options.shaders.fragment && this.options.shaders.fragment.code.indexOf(sampler.name) !== -1 || this.options.shaders && this.options.shaders.compute && this.options.shaders.compute.code.indexOf(sampler.name) !== -1) this.texturesBindGroup.addSampler(sampler);
	}
	/**
	...
	*/
	async getBufferBindingResultByBindingName(bindingName = "") {
		...
		if (binding && "buffer" in binding) {
			const dstBuffer = this.renderer.copyBufferToBuffer({ srcBuffer: binding.buffer });
	...
	*/
	async getBufferElementResultByNames({ bindingName, bufferElementName }) {
		...
		if (!bufferElementName || result.length) return result;
		else {
			const binding = this.getBufferBindingByName(bindingName);
			if (binding) return binding.extractBufferElementDataFromBufferResult({
				result,
				bufferElementName
			});
			else return result;
