...
//#region src/core/renderPasses/RenderPass.ts
/**
...
var RenderPass = class {
	...
	*/
	constructor(renderer, { label = "Render Pass", sampleCount = 4, qualityRatio = 1, fixedSize = null, useColorAttachments = true, renderToSwapChain = true, colorAttachments = [], useDepth = true, depthTexture = null, forceDepthLoadOp = null, forceDepthStoreOp = null, depthClearValue = 1, depthFormat = "depth24plus", forceDepthReadOnly = null, stencilClearValue = 0, forceStencilLoadOp = null, forceStencilStoreOp = null, forceStencilReadOnly = null } = {}) {
		...
		if (useColorAttachments) {
			...
			if (!colorAttachments.length) colorAttachments = [defaultColorAttachment];
			else colorAttachments = colorAttachments.map((colorAttachment) => {
				return {
		...
		if (this.renderer.device) this.init();
	}
	/**
	* Initialize the {@link RenderPass} textures and descriptor.
	*/
	init() {
		if (this.options.useDepth) this.createDepthTexture();
		this.viewTextures = [];
		this.resolveTargets = [];
	...
	*/
	setRenderer(renderer) {
		if (this.renderer) this.renderer.renderPasses.delete(this.uuid);
		renderer = isRenderer(renderer, this.options.label + " " + this.type);
		this.renderer = renderer;
		if (this.options.useDepth && !this.options.depthTexture) this.depthTexture.setRenderer(this.renderer);
		this.viewTextures.forEach((texture) => {
			texture.setRenderer(this.renderer);
		});
		this.resolveTargets.forEach((texture) => {
			if (texture) texture.setRenderer(this.renderer);
		});
		this.renderer.renderPasses.set(this.uuid, this);
	}
	/**
	...
	*/
	createDepthTexture() {
		...
		} else this.depthTexture = new Texture(this.renderer, {
		...
		if (this.depthTexture.options.format.includes("stencil")) this.#useStencil = true;
	}
	/**
	...
	*/
	createViewTextures() {
		this.options.colorAttachments.forEach((colorAttachment, index) => {
			this.viewTextures.push(new Texture(this.renderer, {
	...
	*/
	createResolveTargets() {
		if (this.options.sampleCount > 1) this.options.colorAttachments.forEach((colorAttachment, index) => {
			this.resolveTargets.push(this.options.renderToSwapChain && index === 0 ? null : new Texture(this.renderer, {
	...
	setRenderPassDescriptor(depthTextureView = null) {
		this.descriptor = {
			...
			colorAttachments: this.options.colorAttachments.map((colorAttachment, index) => {
				return {
					view: this.viewTextures[index]?.texture.createView({ label: this.viewTextures[index]?.texture.label + " view" }),
					...this.resolveTargets.length && { resolveTarget: this.resolveTargets[index]?.texture.createView({ label: this.resolveTargets[index]?.texture.label + " view" }) },
					clearValue: colorAttachment.clearValue,
					loadOp: colorAttachment.loadOp,
					storeOp: colorAttachment.storeOp,
					...colorAttachment.depthSlice !== void 0 && { depthSlice: colorAttachment.depthSlice }
				};
			}),
			...this.options.useDepth && { depthStencilAttachment: {
				view: depthTextureView || this.depthTexture.texture.createView({ label: this.depthTexture.texture.label + " view" }),
	...
	*/
	#updateDepthAttachmentSettings() {
		if (this.options.useDepth && this.descriptor.depthStencilAttachment) this.descriptor.depthStencilAttachment = {
	...
	*/
	beginRenderPass(commandEncoder, descriptor = this.descriptor) {
		...
		if (this.viewport) pass.setViewport(this.viewport.left, this.viewport.top, this.viewport.width, this.viewport.height, this.viewport.minDepth, this.viewport.maxDepth);
		if (this.scissorRect) pass.setScissorRect(this.scissorRect.left, this.scissorRect.top, this.scissorRect.width, this.scissorRect.height);
		return pass;
	}
	/**
	...
	*/
	setQualityRatio(qualityRatio = 1) {
		...
		if (!this.options.depthTexture && this.options.useDepth) this.depthTexture.setQualityRatio(this.options.qualityRatio);
		this.resize();
	}
	/**
	...
	*/
	resize() {
		...
		if (this.options.useDepth) this.descriptor.depthStencilAttachment.view = this.depthTexture.texture.createView({ label: this.depthTexture.options.label + " view" });
		this.viewTextures.forEach((viewTexture, index) => {
			this.descriptor.colorAttachments[index].view = viewTexture.texture.createView({ label: viewTexture.options.label + " view" });
		});
		this.resolveTargets.forEach((resolveTarget, index) => {
			if (resolveTarget) this.descriptor.colorAttachments[index].resolveTarget = resolveTarget.texture.createView({ label: resolveTarget.options.label + " view" });
		});
	}
	/**
	...
	*/
	setLoadOp(loadOp = "clear", colorAttachmentIndex = 0) {
		if (this.options.useColorAttachments) {
			if (this.options.colorAttachments[colorAttachmentIndex]) this.options.colorAttachments[colorAttachmentIndex].loadOp = loadOp;
			if (this.descriptor) {
				if (this.descriptor.colorAttachments && this.descriptor.colorAttachments[colorAttachmentIndex]) this.descriptor.colorAttachments[colorAttachmentIndex].loadOp = loadOp;
			}
		}
	}
	/**
	...
	*/
	setClearValue(clearValue = [
		...
		if (this.options.useColorAttachments) {
			...
			if (this.options.colorAttachments[colorAttachmentIndex]) this.options.colorAttachments[colorAttachmentIndex].clearValue = clearValue;
			if (this.descriptor) {
				if (this.descriptor.colorAttachments && this.descriptor.colorAttachments[colorAttachmentIndex]) this.descriptor.colorAttachments[colorAttachmentIndex].clearValue = clearValue;
			}
		}
	}
	/**
	...
	*/
	updateView(renderTexture = null) {
		if (!this.options.colorAttachments.length || !this.options.renderToSwapChain) return renderTexture;
		if (!renderTexture) {
	...
	*/
	destroy() {
		...
		if (!this.options.depthTexture && this.depthTexture) this.depthTexture.destroy();
